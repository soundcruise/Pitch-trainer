...
### 👑 Pro版 (Pro Version)
- **URL（会員向け・PWA用）**: `https://soundcruise.github.io/Pitch-trainer/prok3m9/`（`pro` の直後に推測しにくい4文字 `k3m9` を付与）
- **旧URL**: `pro.html` および以前の `pro/` パスは **開けない（無効）** ようにしてあります。会員の方のみ、共有されたURLをご利用ください。
...
1.  **通常版は `…/standard/`、Pro版は会員に共有した `…/prok3m9/`** をスマートフォンのブラウザ（iPhoneならSafari、AndroidならChrome）で開きます（Android で2つ別アプリにするためにパスを分けています）。
...
- **キャッシュの更新**:
    反映後も古い画面のままなら強制再読み込みを試すか、`standard/service-worker.js` と `prok3m9/service-worker.js` の `CACHE_NAME` を変えてデプロイすると更新が伝わりやすくなります。
...
- **共通ロジック**: `script.js`
- **共通スタイル**: `style.css`
- **入口（リダイレクトのみ）**: ルートの `index.html` → `standard/`。`pro.html` はリダイレクトせず利用終了の案内のみ表示します。
- **通常版HTML**: `standard/index.html`（マニフェスト・SW は `standard/` 内）
- **Pro版HTML**: `prok3m9/index.html`（同上）
