...
- **短い入口（ルート）**: `pro.html`（例: `https://soundcruise.jp/pro.html`）は **`apps/pitch-cruise/pro_x9v7q2m8/`** へ転送します。
...
- **入口**: ルートの `index.html` は **404 表示**。`beta.html` → **`apps/pitch-cruise/beta/`**。`pro.html` → **`apps/pitch-cruise/pro_x9v7q2m8/`**。`staging.html` → **`apps/pitch-cruise/staging/?k=…`**。ルートの `beta/`・`staging/`・`pro_x9v7q2m8/` は旧パスからの転送のみ。`clear.html` はキャッシュ削除後 **`apps/pitch-cruise/standard/`** へ。
