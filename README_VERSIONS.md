...
1.  **通常版は `…/standard/`、Pro版は会員に共有した `…/pro_k3m9/`** をスマートフォンのブラウザ（iPhoneならSafari、AndroidならChrome）で開きます（Android で2つ別アプリにするためにパスを分けています）。
...
- **キャッシュの更新**:
    反映後も古い画面のままなら強制再読み込みを試すか、`standard/service-worker.js` と `pro_k3m9/service-worker.js` の `CACHE_NAME` を変えてデプロイすると更新が伝わりやすくなります。
