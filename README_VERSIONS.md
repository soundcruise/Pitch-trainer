...
- **独自ドメイン（DNS 反映後）**: `https://pitch-trainer.soundcruise.jp/standard/`（設定手順は `URL_manual/URL_manual.txt` の「1-b」参照）
- **ルート**: [https://soundcruise.github.io/Pitch-trainer/](https://soundcruise.github.io/Pitch-trainer/) は **404 表示**（自動で `standard/` へは移動しません）
...
- **共通ロジック**: `script.js`
- **共通スタイル**: `style.css`
- **入口**: ルートの `index.html` は **404 表示**。`beta.html` → `beta/`。`pro.html` はリダイレクトせず利用終了の案内のみ表示します。
